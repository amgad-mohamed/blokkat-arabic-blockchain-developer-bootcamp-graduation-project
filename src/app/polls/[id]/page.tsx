...
const Poll = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  useEffect(() => {
    if (isConfirming) toast.loading("Transaction pending...", { id: "tx-status" });
    if (isConfirmed) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!selectedOption || !pollId || !isActive) {
      if (!isActive) toast.error("This poll is no longer active or has expired.");
      return;
    }
        ...
        <CardHeader className="flex flex-col items-center">
          ...
          {poll?.expiresAt && (
            <p className="text-gray-500 text-sm">
              Expires: {new Date(Number(poll.expiresAt) * 1000).toLocaleString()}
            </p>
          )}
        </CardHeader>
        <form onSubmit={handleSubmit}>
          ...
          <CardFooter className="pt-8">
            <Button
              ...
              variant="customBlue"
              className="w-full"
              disabled={!selectedOption || isSubmitting || isConfirming || !isActive}
            >
